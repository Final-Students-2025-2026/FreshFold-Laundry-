 ...
 *
 * A module of its own, and the separation is the point. The console keeps every
 * other thing it caches through AsyncStorage directly, which is an unencrypted
 * store in the app sandbox — a SQLite file on
 * Android, a plist-backed file on iOS — and the session token was sitting in
 * it. That token is good for a week and reports this courier's position,
 * accepts jobs and completes hand-offs, so it was readable from an
 * unencrypted device backup, from any process on a rooted or jailbroken
 * handset, and over `adb` on a debuggable build.
 *
 * Everything else the console caches — the board, messages, the courier record —
 * stays where it was. Those are a copy of what the token can
 ...
 * remember for the wrong reason — and here it would strand couriers mid-shift.
 *
 ...
 * the token simply is not persisted: the courier stays signed in for this
 * session, because the token is in memory, and signs in again next launch.
 * That is a worse start to a shift than a silent write to AsyncStorage would be, and it
